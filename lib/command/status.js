...
class Status {
  ...
  handleMessage (talker) {
    ...
    this.jira.findIssue(issueId)
            .then((issue) => {
              ...
              const responseFormat = talker.message.event === 'direct_mention' ? RESPONSE_FULL : null;
              response.attachments = [this.issueResponse(issue, responseFormat)];
              talker.sayResponse(response);
            })
        .catch((error) => {
  ...
  shortHelp() {
    return '*status*: get the current status of a given issue';
  }
}
