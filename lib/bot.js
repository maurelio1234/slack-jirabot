...
const Review = require('./command/review.js')
const Done = require('./command/done.js')
...
class Bot {
  ...
  constructor (config) {
    ...
    this.commands.push(new Start(this.jira, this.config))
    this.commands.push(new Review(this.jira, this.config))
    this.commands.push(new Done(this.jira, this.config))
  }
    ...
    if (message.type === 'message' && message.text) {
      ...
      for(command of this.commands) {
        if (command.canHandleMessage(message)) {
          command.handleMessage(this.bot, message, response);
          found = true;
          break;
        }
      }
